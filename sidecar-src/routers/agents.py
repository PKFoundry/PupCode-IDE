...
async def list_agents():
    ...
    try:
        ...
        return {"agents": [], "active_agent": None, "error": str(e)}
...
async def create_agent(body: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"Created JSON agent: {agent_name}")
...
async def update_agent(name: str, body: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"Updated JSON agent: {name}")
...
async def delete_agent(name: str):
    ...
    try:
        ...
        agent_file = Path(json_agents[name])
        agent_file.unlink()
        logger.info(f"Deleted JSON agent: {name}")
        return {"success": True, "name": name}
    except Exception as e:
